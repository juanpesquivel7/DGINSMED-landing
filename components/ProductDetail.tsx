      ...
      <div className="grid gap-10 sm:grid-cols-2">
        ...
        <div className="flex flex-col gap-4">
          <span className="w-fit rounded-full bg-brand-500/15 px-3 py-1 text-xs font-semibold text-brand-400">
            {product.applicatorType === "automatic"
              ? t("applicatorAutomatic")
              : t("applicatorSemiAutomatic")}
          </span>
          <h1 className="text-2xl font-semibold text-foreground">
            {product.name}
          </h1>
          <p className="text-foreground/70">{product.description[locale]}</p>
          ...
          <dl className="mt-4 grid gap-3 border-t border-border pt-4 text-sm">
            <div>
              <dt className="font-semibold text-foreground">
                {t("productSyringeType")}
              </dt>
              <dd className="text-foreground/70">{product.syringeType[locale]}</dd>
            </div>
            <div>
              <dt className="font-semibold text-foreground">
                {t("productTreatments")}
              </dt>
              <dd className="text-foreground/70">{product.treatments[locale]}</dd>
            </div>
          </dl>
          ...
          <div>
            ...
            <ul className="flex flex-col gap-2 text-sm text-foreground/70">
              {product.features.map((feature) => (
                <li key={feature.es} className="flex gap-2">
          ...
          <p className="text-xs text-foreground/40">
            {t("productWeight")}: {product.weight} {product.weightUnit} ·{" "}
            {t("productDimensions")}: {product.dimensions.length}x
            {product.dimensions.width}x{product.dimensions.height}{" "}
            {product.dimensions.lengthUnit}
          </p>
        </div>
      </div>
    </div>
